...
class Event_Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        #self.reduce_temp_dimentation = 1024 
        self.final_temp_dimentation = 2048 
        #self.final_temp_dimentation = 1024 

        self.dilatednonlocal_redudim_bn1 = nn.BatchNorm1d(self.concept_number*3)
        self.dilatednonlocal_reduce_dim = nn.Linear(self.concept_number*3, self.final_temp_dimentation)

        self.coattention_redudim_bn1 = nn.BatchNorm1d(self.concept_number*2)
        self.coattention_reduce_dim = nn.Linear(self.concept_number*2, self.final_temp_dimentation)
        
        #self.concat_bn1 = nn.BatchNorm1d(self.reduce_temp_dimentation*2)
        #self.concat_reduce_dim = nn.Linear(self.reduce_temp_dimentation*2, self.final_temp_dimentation)
        #self.concat_bn1 = nn.BatchNorm1d(self.concept_number*5)
        #self.concat_reduce_dim = nn.Linear(self.concept_number*5, self.final_temp_dimentation)

        #### classification
        self.dropout = nn.Dropout(0.5)
        self.final_bn1 = nn.BatchNorm1d(self.final_temp_dimentation)
        self.final_classifier = nn.Linear(self.final_temp_dimentation, opt.event_classes)
    ...
    def forward(self, sceobj_frame):
        ...
        #### feature concat & reduce dim
        ## dilatednonlocal feature concat & reduce dim
        ...
        dilatednonlocal_feature = self.dilatednonlocal_redudim_bn1(dilatednonlocal_feature)
        dilatednonlocal_feature = self.reduce_dropout(dilatednonlocal_feature)
        dilatednonlocal_feature = self.dilatednonlocal_reduce_dim(dilatednonlocal_feature)
       
        ## coattention feature concat & reduce dim
        ...
        coattention_feature = self.coattention_redudim_bn1(coattention_feature)
        coattention_feature = self.reduce_dropout(coattention_feature)
        coattention_feature = self.coattention_reduce_dim(coattention_feature)

        #### reduce dimentation
        concat_feature = dilatednonlocal_feature + coattention_feature
        #concat_feature = torch.cat((dilatednonlocal_feature, coattention_feature), 1) 
        #concat_feature = F.relu(concat_feature)
        ...
        #### classification
        final_classification = F.relu(concat_feature)
