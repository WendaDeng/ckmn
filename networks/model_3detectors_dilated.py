...
class Event_Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        #self.scene_dilated_1 = nn.Conv1d(in_channels=self.T, out_channels=self.T, kernel_size=3, stride=1, padding=1, dilation=1)
        #self.scene_dilated_2 = nn.Conv1d(in_channels=self.T, out_channels=self.T, kernel_size=3, stride=1, padding=2, dilation=2)
        #self.scene_dilated_4 = nn.Conv1d(in_channels=self.T, out_channels=self.T, kernel_size=3, stride=1, padding=4, dilation=4)

        #self.object_dilated_1 = nn.Conv1d(in_channels=self.T, out_channels=self.T, kernel_size=3, stride=1, padding=1, dilation=1)
        #self.object_dilated_2 = nn.Conv1d(in_channels=self.T, out_channels=self.T, kernel_size=3, stride=1, padding=2, dilation=2)
        #self.object_dilated_4 = nn.Conv1d(in_channels=self.T, out_channels=self.T, kernel_size=3, stride=1, padding=4, dilation=4)

        #self.action_dilated_1 = nn.Conv1d(in_channels=self.T, out_channels=self.T, kernel_size=3, stride=1, padding=1, dilation=1)
        #self.action_dilated_2 = nn.Conv1d(in_channels=self.T, out_channels=self.T, kernel_size=3, stride=1, padding=2, dilation=2)
        #self.action_dilated_4 = nn.Conv1d(in_channels=self.T, out_channels=self.T, kernel_size=3, stride=1, padding=4, dilation=4)
    ...
    def forward(self, sceobj_frame):
        ...
        #scene_feature_dilated = torch.cat((torch.cat((scene_feature_dilated_1, scene_feature_dilated_2), 2), scene_feature_dilated_4), 2)
        ...
        #object_feature_dilated = torch.cat((torch.cat((object_feature_dilated_1, object_feature_dilated_2), 2), object_feature_dilated_4), 2)
        ...
        #action_feature_dilated = torch.cat((torch.cat((action_feature_dilated_1, action_feature_dilated_2), 2), action_feature_dilated_4), 2)

        #### concat & classification
        ## max pooling
        #scene_feature_dilated = scene_feature_dilated.permute(0, 2, 1)
        #object_feature_dilated = object_feature_dilated.permute(0, 2, 1)
        #action_feature_dilated = action_feature_dilated.permute(0, 2, 1)

        #scene_feature, _ = torch.max(scene_feature_dilated, dim=1)
        #object_feature, _ = torch.max(object_feature_dilated, dim=1)
        #action_feature, _ = torch.max(action_feature_dilated, dim=1)
