...
class Event_Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        #### general feature 
        self.general_feature_model = model_general.General_Model(opt)
        ...
        self.trilinear_dropout = nn.Dropout(0.5)
        self.coattention_trilinear = nn.Linear(self.concept_number*2, self.trilinear_dim)
        self.dilatednonlocal_trilinear = nn.Linear(self.concept_number*3, self.trilinear_dim)
        self.generalfeature_trilinear = nn.Linear(self.general_feature_dim, self.trilinear_dim)
    ...
    def forward(self, sceobj_frame, action_frame):
        ...
        #### general feature 
        general_feature = self.general_feature_model(action_frame)
        ...
        ## pooling
        general_feature = F.relu(general_feature)
        dilatednonlocal_feature = F.relu(dilatednonlocal_feature)
        coattention_feature = F.relu(coattention_feature)
        
        general_feature = self.generalfeature_trilinear(general_feature)
        dilatednonlocal_feature = self.dilatednonlocal_trilinear(dilatednonlocal_feature)
        coattention_feature = self.coattention_trilinear(coattention_feature)
        
        bilinear_feature = torch.mul(torch.mul(dilatednonlocal_feature, coattention_feature), general_feature)
