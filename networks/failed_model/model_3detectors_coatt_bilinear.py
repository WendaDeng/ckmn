...
class Event_Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        #### classification
        self.concat_reduce_dim = nn.Linear(self.bilinear_dim*3, self.bilinear_dim)
    ...
    def forward(self, sceobj_frame, action_frame):
        ...
        ## scene & object & action
        concat_representation = torch.cat((torch.cat((scob_bilinear, scac_bilinear), dim=1), obac_bilinear) ,dim=1) 
        # concat_representation = F.normalize(concat_representation) 
        concat_representation = self.concat_reduce_dim(concat_representation)

        #### classification
        final_classification = self.relu(concat_representation)
