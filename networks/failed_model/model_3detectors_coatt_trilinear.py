...
class Event_Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        #self.trilinear_reduce_dim = 4096 
        self.scene_trilinear = nn.Linear(self.scene_classes*2, self.trilinear_dim)
        self.object_trilinear = nn.Linear(self.object_classes*2, self.trilinear_dim)
        self.action_trilinear = nn.Linear(self.action_classes*2, self.trilinear_dim)
        
        #self.trilinear_avgpool = nn.AdaptiveAvgPool1d(self.trilinear_reduce_dim)
        
        #self.scob_trilinear_bn1 = nn.BatchNorm1d(self.scene_classes + self.object_classes)
        #self.scac_trilinear_bn1 = nn.BatchNorm1d(self.scene_classes + self.action_classes)
        #self.obac_trilinear_bn1 = nn.BatchNorm1d(self.object_classes + self.action_classes)

        #self.scob_trilinear = nn.Linear(self.scene_classes + self.object_classes, self.trilinear_dim)
        #self.scac_trilinear = nn.Linear(self.scene_classes + self.action_classes, self.trilinear_dim)
        #self.obac_trilinear = nn.Linear(self.object_classes + self.action_classes, self.trilinear_dim)

        ##### feature reduce dimentation & fusion
        #self.reduce_dim = 1024
        #self.concat_bn1 = nn.BatchNorm1d(self.trilinear_reduce_dim)
        #self.concat_reduce_dim = nn.Linear(self.trilinear_reduce_dim, self.reduce_dim)
        ### classification
    ...
    def forward(self, sceobj_frame, action_frame):
        ...
        #scobac_trilinear = self.trilinear_avgpool(scobac_trilinear.unsqueeze(1)).squeeze(1)
        ...
        #scob_feature = torch.cat((scene_feature_scob, object_feature_scob), 1)
        #scac_feature = torch.cat((scene_feature_scac, action_feature_scac), 1)
        #obac_feature = torch.cat((object_feature_obac, action_feature_obac), 1)
        
        #scob_feature = F.relu(scob_feature)
        #scob_feature = self.scob_trilinear_bn1(scob_feature)
        
        #scac_feature = F.relu(scac_feature)
        #scac_feature = self.scac_trilinear_bn1(scac_feature)
        
        #obac_feature = F.relu(obac_feature)
        #obac_feature = self.obac_trilinear_bn1(obac_feature)

        #scob_trilinear = self.scob_trilinear(scob_feature)
        #scac_trilinear = self.scac_trilinear(scac_feature)
        #obac_trilinear = self.obac_trilinear(obac_feature)
        
        #scobac_trilinear = torch.mul((torch.mul(scob_trilinear, scac_trilinear)), obac_trilinear)
        #scobac_trilinear = torch.sign(scobac_trilinear)*torch.sqrt(torch.abs(scobac_trilinear)+1e-10)
        #scobac_trilinear = F.normalize(scobac_trilinear)

        #### classification
        #final_classification = self.relu(scobac_trilinear)
        #final_classification = self.concat_bn1(final_classification)
        #final_classification = self.dropout(final_classification)
        #final_classification = self.concat_reduce_dim(final_classification)

        #final_classification = self.relu(final_classification)
        final_classification = self.relu(scobac_trilinear)
