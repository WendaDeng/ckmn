...
class Event_Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        #### 1dconv
        self.scene_num_clusters = 256 
        self.scene_conv = nn.Conv1d(self.scene_classes, self.scene_num_clusters, kernel_size=(1), bias=True)
        
        self.object_num_clusters = 256
        self.object_conv = nn.Conv1d(self.object_classes, self.object_num_clusters, kernel_size=(1), bias=True)

        self.action_num_clusters = 256
        self.action_conv = nn.Conv1d(self.action_classes, self.action_num_clusters, kernel_size=(1), bias=True)
    ...
    def forward(self, sceobj_frame, action_frame):
        ...
        #### 1dconv 
        ## normalize
        scene_feature_t = scene_feature.permute(0, 2, 1).contiguous()
        object_feature_t = object_feature.permute(0, 2, 1).contiguous()
        action_feature_t = action_feature.permute(0, 2, 1).contiguous()
        ...
        scene_conv = self.scene_conv(scene_feature_t)
        object_conv = self.object_conv(object_feature_t)
        action_conv = self.action_conv(action_feature_t)
        
       
        ## max pooling
        scene_feature_encod, _ = torch.max(scene_conv, dim=2)
        object_feature_encod, _ = torch.max(object_conv, dim=2)
        action_feature_encod, _ = torch.max(action_conv, dim=2)
