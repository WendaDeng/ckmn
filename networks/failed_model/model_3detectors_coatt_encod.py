...
class Event_Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        ####  encoding 
        self.dict_dim = 128
        self.scene_encoding = encoding.nn.Encoding(2, self.dict_dim)
        self.object_encoding = encoding.nn.Encoding(2, self.dict_dim)
        self.action_encoding = encoding.nn.Encoding(2, self.dict_dim)

        #### feature fusion
        self.common_dimentation = 512
        self.reduce_dropout = nn.Dropout(0.5)
        self.concat_reduce_dim = nn.Linear(self.dict_dim*2*3, self.common_dimentation)
    ...
    def forward(self, sceobj_frame, action_frame):
        ...
        #### encoding
        scene_encoding = self.scene_encoding(scene_feature_co).view(N, -1)
        object_encoding = self.object_encoding(object_feature_co).view(N, -1)
        action_encoding = self.action_encoding(action_feature_co).view(N, -1)
        concat_feature = torch.cat((torch.cat((scene_encoding, object_encoding), 1), action_encoding), 1) 
