...
class Event_Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.bilinear_dim = 1024 
        self.bilinear_dropout = 0.1
        self.scene_bilinear_scob = nn.Linear(self.scene_classes, self.bilinear_dim)
        self.object_bilinear_scob = nn.Linear(self.object_classes, self.bilinear_dim)
        self.scene_bilinear_scac = nn.Linear(self.scene_classes, self.bilinear_dim)
        self.action_bilinear_scac = nn.Linear(self.action_classes, self.bilinear_dim)
        self.object_bilinear_obac = nn.Linear(self.object_classes, self.bilinear_dim)
        self.action_bilinear_obac = nn.Linear(self.action_classes, self.bilinear_dim)

        self.scene_bilinear_scob_bn1 = nn.BatchNorm1d(self.scene_classes)
        self.object_bilinear_scob_bn1 = nn.BatchNorm1d(self.object_classes)
        self.scene_bilinear_scac_bn1 = nn.BatchNorm1d(self.scene_classes)
        self.action_bilinear_scac_bn1 = nn.BatchNorm1d(self.action_classes)
        self.object_bilinear_obac_bn1 = nn.BatchNorm1d(self.object_classes)
        self.action_bilinear_obac_bn1 = nn.BatchNorm1d(self.action_classes)

        self.trilinear_dim = 2048 
        self.trilinear_dropout = 0.1
        self.scob_trilinear = nn.Linear(self.bilinear_dim, self.trilinear_dim)
        self.scac_trilinear = nn.Linear(self.bilinear_dim, self.trilinear_dim)
        self.obac_trilinear = nn.Linear(self.bilinear_dim, self.trilinear_dim)

        self.scob_trilinear_bn1 = nn.BatchNorm1d(self.bilinear_dim)
        self.scac_trilinear_bn1 = nn.BatchNorm1d(self.bilinear_dim)
        self.obac_trilinear_bn1 = nn.BatchNorm1d(self.bilinear_dim)
    ...
    def forward(self, sceobj_frame, action_frame):
        ...
        ## scene & object 
        scene_feature_scob = F.relu(scene_feature_scob)
        object_feature_scob = F.relu(object_feature_scob)

        scene_feature_scob = self.scene_bilinear_scob_bn1(scene_feature_scob)
        object_feature_scob = self.object_bilinear_scob_bn1(object_feature_scob)
        ...
        scob_bilinear = torch.sign(scob_bilinear)*torch.sqrt(torch.abs(scob_bilinear)+1e-10)
        scob_bilinear = F.normalize(scob_bilinear)

        ## scene & action 
        scene_feature_scac = F.relu(scene_feature_scac)
        action_feature_scac = F.relu(action_feature_scac)

        scene_feature_scac = self.scene_bilinear_scac_bn1(scene_feature_scac)
        action_feature_scac = self.action_bilinear_scac_bn1(action_feature_scac)
        ...
        scac_bilinear = torch.sign(scac_bilinear)*torch.sqrt(torch.abs(scac_bilinear)+1e-10)
        scac_bilinear = F.normalize(scac_bilinear)

        ## object & action
        object_feature_obac = F.relu(object_feature_obac)
        action_feature_obac = F.relu(action_feature_obac)
        
        object_feature_obac = self.object_bilinear_obac_bn1(object_feature_obac)
        action_feature_obac = self.action_bilinear_obac_bn1(action_feature_obac)
        ...
        obac_bilinear = torch.sign(obac_bilinear)*torch.sqrt(torch.abs(obac_bilinear)+1e-10)
        obac_bilinear = F.normalize(obac_bilinear)

        ## scene & object & action
        scob_bilinear = F.relu(scob_bilinear)
        scob_bilinear = self.scob_trilinear_bn1(scob_bilinear)
        
        scac_bilinear = F.relu(scac_bilinear)
        scac_bilinear = self.scac_trilinear_bn1(scac_bilinear)
        
        obac_bilinear = F.relu(obac_bilinear)
        obac_bilinear = self.obac_trilinear_bn1(obac_bilinear)
