...
class Event_Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        #### self attention
        self.concept_common = 128
        self.scene_attention = Parameter(torch.Tensor(self.scene_classes, self.concept_common))
        self.object_attention = Parameter(torch.Tensor(self.object_classes, self.concept_common))
        self.action_attention = Parameter(torch.Tensor(self.action_classes, self.concept_common))

        #### feature fusion
        self.common_dimentation = 512
        self.reduce_dropout = nn.Dropout(0.5)
        self.concat_reduce_dim = nn.Linear(self.concept_common*2*3, self.common_dimentation)

        #### classification 
        self.final_classifier = nn.Linear(self.common_dimentation, opt.event_classes)
    ...
    def _reset_parameters(self):
        ...
        self.scene_attention.data.uniform_(-stdv, stdv)
        self.object_attention.data.uniform_(-stdv, stdv)
        self.action_attention.data.uniform_(-stdv, stdv)
    ...
    def forward(self, sceobj_frame, action_frame):
        ...
        scene_feature_co = torch.cat((scene_feature_scac.unsqueeze(1), scene_feature_scob.unsqueeze(1)), 1)
        object_feature_co = torch.cat((object_feature_scob.unsqueeze(1), object_feature_obac.unsqueeze(1)), 1)
        action_feature_co = torch.cat((action_feature_scac.unsqueeze(1), action_feature_obac.unsqueeze(1)), 1)
        
        #### self attention
        scene_attention = torch.matmul(scene_feature_co, self.scene_attention).view(N, -1)
        object_attention = torch.matmul(object_feature_co, self.object_attention).view(N, -1)
        action_attention = torch.matmul(action_feature_co, self.action_attention).view(N, -1)

        #### reduce dimentation
        concat_feature = torch.cat((torch.cat((scene_attention, object_attention), 1), action_attention), 1)
